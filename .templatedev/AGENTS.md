<!-- generiert von .templatedev/scripts/sync-rules.py aus ../AGENTS.md – nicht bearbeiten, Änderungen im Template -->

# AGENTS.md — anbieterneutrale Regeln für Template-Pflege

> Platzhalter (`Template-Pflege`, `Wolfgang`, `Opus`, `Python 3 + Markdown (Agentic-Coding-Template)`) werden beim Anlegen des
...
| :--- | :--- | :--- |
| `Template-Pflege` | Name des Projekts | „Beispiel-App" |
| `Wolfgang` | Mensch, der das Projekt verantwortet und entscheidet | „Wolfgang" |
| `Opus` | Rufname des jeweils genutzten Haupt-Assistenten — standardmäßig der Kurzname dessen, was arbeitet: das Modell, wo es wählbar ist, sonst das Werkzeug | „Opus“, „Sonnet“, „Gemini“ |
| `Python 3 + Markdown (Agentic-Coding-Template)` | Technologie-Stack in Kurzform | „Nuxt 4 + MariaDB" |
...
- **Orchestrator** — der jeweils genutzte Haupt-Assistent (Claude Code im Hauptfenster, ein ChatGPT-/Codex-Chat,
  ...
  **Der Rufname `Opus` ist eine Anrede, keine Bedingung.** Angesprochen ist der Orchestrator
  ...
  Wolfgang gibt es niemanden sonst.
  Nennt Wolfgang dagegen eine **Worker-Rolle** („lass den Explorer nachsehen", „builder soll das
...
- **Auftraggeber** — Wolfgang, der Mensch, der Ziele setzt, Fragen beantwortet und kritische Schritte
  freigibt.
...
- **`AI-CONFIG.md` im Repo-Root** steuert die Zusammenarbeit, und zwar **beide Abschnitte laufend**. Der
  ...
  Wolfgang ändert dort jederzeit, ohne dass jemand Code anfassen muss.
- **Geänderte Konfiguration wird umgesetzt, nicht nur vermerkt.** `python .claude/scripts/sync-config.py
  ...
  und braucht die Zusage von Wolfgang. Der zuletzt umgesetzte Stand steht in `.claude/template.json`
...
- Menschlicher Originaltext in `docs/ai/` (Fragen, Antworten, Kommentare von Wolfgang) ist unantastbar:
  nie editieren oder löschen, nur darunter kommentieren.
- **Tabu-Bereich:** Der Abschnitt „Aufgaben nur für Wolfgang" in `docs/ai/tasks.md` wird von keinem
  ...
  präzisiert oder als erledigt markiert werden, sobald Wolfgang es meldet. Jede Aufgabe dort hat eine
  eigene `* Antwort:`-Zeile: Wolfgang meldet darin die Erledigung (mit Zusatzinfos zur Umsetzung),
...
- **Keine Standardantwort annehmen:** Eine Frage an Wolfgang gilt erst als beantwortet, wenn er
...
- **Eine Idee wird erst analysiert, dann gebaut.** Äußert Wolfgang ein Feature, eine Idee oder einen
  ...
  Abkürzung wird aber ausgesprochen, damit Wolfgang widersprechen kann.
- **Rückfragen gehören an den Anfang, nicht in die laufende Umsetzung.** Bevor ein größerer Block oder ein
  ...
  **Sofort gefragt wird nur in drei Fällen:** (1) Die Aufgabe wurde ausdrücklich als Gespräch gestartet
  ...
  Für **Worker** gilt die Regel verschärft: Sie fragen Wolfgang nie selbst, sondern geben
...
4. Bleibt sie geflaggt und betrifft sie den Tabu-Bereich (siehe oben): als Aufgabe mit Rezept (Kontext + genauer
   Schritt) unter „Aufgaben nur für Wolfgang" ablegen statt zu erzwingen.
...
Wolfgang gemeldet, mit dem, was sie verhindert hat. Er entscheidet über Berechtigungsmodus oder ein
...
- **Schreiben nur mit ausdrücklicher, datierter Freigabe** von Wolfgang für genau diesen Zweck. Die
...
- **Pfade außerhalb des Projekts plattformgerecht nennen.** Nutzerverzeichnis, App- und MCP-Konfigurationen
  unterscheiden sich je Betriebssystem. Im Gespräch gilt der Pfad der Plattform, auf der Wolfgang
...
Wolfgang ändert die beiden Werte direkt hier in `AGENTS.md`; die Schreibmechanik liest genau diesen
...
- `agent`: wer handelt — `orchestrator`, `user` (Wolfgang), sonst der Worker-Name (`builder`,
...
| `INFO` | Sitzungsstart/-ende · jede Eingabe von Wolfgang (gekürzt) · **jede Entscheidung des Orchestrators** (was, warum, an wen) · **Start und Ende jedes Workers** mit Auftrag bzw. Ergebnis-Kurzfassung · Review-Urteile · Doku prüfen und nachziehen · Commits |
...
kein Sonderfall der Automatik, sondern das Gegenteil davon: Wolfgang formuliert selbst und löst
...
So funktioniert es, wenn Wolfgang zustimmt:
...
Die Einrichtung selbst (Anlegen oder Nachrüsten) gilt erst als abgeschlossen, wenn Wolfgang das
