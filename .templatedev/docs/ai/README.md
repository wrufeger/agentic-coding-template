<!-- generiert von .templatedev/scripts/sync-rules.py aus ../docs/ai/README.md – nicht bearbeiten, Änderungen im Template -->

> Datenstand: 2026-09-17 – Status: Entwurf

# docs/ai/ — Zusammenarbeit Mensch/Assistent

Dieser Ordner ist die Arbeitsfläche der Zusammenarbeit zwischen Wolfgang (Auftraggeber) und dem
...
- Wolfgang schreibt frei in `tasks.md` (neue Aufgaben), `questions.md` (Antworten unter den Fragen) und
...
Jede Antwort von Wolfgang — in `questions.md`, in den `* Antwort:`-Zeilen von `tasks.md` oder in
...
Keine Sammelverbuchung am Dateiende: Wolfgang muss an der einzelnen Stelle sehen, ob seine Antwort
...
| `tasks.md` | Aufgabenliste inkl. Tabu-Abschnitt „nur für Wolfgang" | vor jeder neuen Aufgabe |
| `tasks_archive.md` | Erledigte Aufgaben im Volltext | bei Bedarf nachschlagen |
| `questions.md` | Offene Fragen/Entscheidungen, die auf Wolfgang warten | nach jeder Antwort verbuchen |
...
| `backlog.md` | Verbesserungsvorschläge, Wolfgang entscheidet inline | vor größeren Umbauten |
...
- **Ausstehende Freigaben** — nur Fragen, die *jetzt* auf eine Antwort von Wolfgang warten, je eine
...
- Wolfgang entscheidet **direkt am Punkt** — „-> machen", „-> später", „-> nein, weil …".
...
| Anweisung | `machen` · `nicht machen` · `offen` — hier entscheidet Wolfgang |
...
**Priorität wird von beiden Seiten vergeben.** Wolfgang kennt den Geschäftswert, Opus den
...
Schreibweise in der Spalte: `⌀ 3,5 (4/3)` — Mittelwert, dahinter Wolfgangs und Opuss
...
`tasks.md` enthält einen Abschnitt „Aufgaben nur für Wolfgang". Dieser Abschnitt wird von **keinem**
Assistenten je ausgeführt (siehe `AGENTS.md`). Einträge dort dürfen ergänzt, präzisiert oder als erledigt
markiert werden, sobald Wolfgang es meldet.
